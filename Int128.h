...
public:
    ...
    //inline _int128 operator%(const int rhs) const {
    //    _int128 rh = (_int128)rhs;
    //    _int128 result, copy(*this);
    //    int128rem(&result, &copy, &rh);
    //    return result;
    //};
...
//inline bool operator>(const _int128& lft, const int rhs) {
//    const _int128 rh = (_int128)rhs;
//    return int128cmp(&lft, &rh) > 0;
//}
inline bool operator>(const _int128& lft, const _uint128& rhs) {
...
std::ostream& operator<<(std::ostream& os, _int128 t) {
    ...
    int i = 0;
    do {
        unsigned int ti = (_uint128)t % (_uint128)1000000000;
        t /= (_int128)1000000000;
        ts[i++] = ti;
    } while (t > (_int128)0);
    int i1 = i - 1;
    while (i-- > 0) {
        if (i < i1 && ts[i] < 100000000)
            for (int j = (ts[i] ? ts[i] : 1); j < 100000000; j *= 10)
                str += "0";
        str += std::to_string(ts[i]);
    }
    return os << str;
}

std::ostream& operator<<(std::ostream& os, _uint128 t) {
    // TODO: Convert t to string
    unsigned int ts[5] = { 0 };
    std::string str = "";
    int i = 0;
    do {
        unsigned int ti = t % (_uint128)1000000000;
        t /= (_uint128)1000000000;
        ts[i++] = ti;
    } while (t > (_uint128)0);
    int i1 = i - 1;
    while (i-- > 0) {
